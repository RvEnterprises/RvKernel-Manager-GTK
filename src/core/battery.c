...
power_supply_list(gsize *count)
{
        ...
        for (gsize i = 0; i < n_entries; i++) {
                ...
                g_ptr_array_add(result,
                                power_supply_new(path, entries[i]));
                g_free(path);
        }
        g_strfreev(entries);
...
void
power_supply_free(PowerSupply *ps)
{
        if (ps == NULL)
                return;
        g_free(ps->path);
...
void
power_supply_list_free(PowerSupply **list, gsize count)
{
        if (list == NULL)
                return;
