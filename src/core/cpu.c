...
static gint cmp_policy_cpus_desc(gconstpointer a, gconstpointer b)
{
...
static gchar *realpath_dup(const gchar *path)
{
...
static gchar **read_token_list(const gchar *dir, const gchar *name)
{
	gchar *text;
	gchar **tokens;
...
static void policy_read_state(CpuPolicy *p)
{
	g_free(p->governor);
...
static void policy_load_freqs(CpuPolicy *p)
{
...
static CpuPolicy *load_policy(const gchar *dir_path)
{
	CpuPolicy *p;
	...
	p->governors = read_token_list(dir_path, "scaling_available_governors");

	if (p->governors[0] == NULL) {
		g_strfreev(p->governors);
		p->governors = g_new0(gchar *, 2);
		p->governors[0] =
			read_first_line_in(dir_path, "scaling_governor");
	}
...
CpuPolicy **cpu_policies(gsize *count)
{
	...
	if (count != NULL)
		*count = result->len;
	log_debug("cpufreq: %u policy devices", (guint)result->len);
	g_ptr_array_add(result, NULL);
	return (CpuPolicy **)g_ptr_array_free(result, FALSE);
}

void cpu_policy_free(CpuPolicy *p)
{
	if (p == NULL)
		return;
	g_free(p->path);
...
gboolean cpu_set_governor(CpuPolicy *policy, const gchar *governor,
			  GError **error)
{
	...
	if (ok)
		...
		log_warn("%s: governor -> %s not applied", policy->path,
			 governor);
	g_free(path);
	return ok;
}

gboolean cpu_set_epp(CpuPolicy *policy, const gchar *preference, GError **error)
{
	...
	if (ok)
		...
		log_warn("%s: EPP -> %s not applied", policy->path, preference);
	g_free(path);
	return ok;
}

gboolean cpu_set_min_freq(CpuPolicy *policy, gint64 khz, GError **error)
{
...
gboolean cpu_set_max_freq(CpuPolicy *policy, gint64 khz, GError **error)
{
