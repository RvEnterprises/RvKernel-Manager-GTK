...
static gchar *read_os_pretty_name(void)
{
	gchar *content;
	gchar **lines;
	gchar *result = NULL;
...
static void read_disk_swap(SystemInfo *info)
{
	gchar *content;
	gchar **lines;
...
static void info_cache_init(void)
{
	struct utsname uts;
	glong cores;
	...
	if (uname(&uts) == 0) {
		...
		info_cache.kernel =
			g_strdup_printf("%s %s", uts.sysname, uts.release);
	...
	log_info("host %s, %s, %s, %u core(s)", info_cache.hostname,
		 info_cache.distro, info_cache.kernel,
		 (guint)info_cache.n_cores);
}
...
SystemInfo *system_info_get(void)
{
	...
	if (sysinfo(&si) == 0) {
		...
		info->mem_available_kb =
			((guint64)si.totalram - (guint64)si.freeram -
			 (guint64)si.bufferram) *
			si.mem_unit / 1024;
		info->swap_total_kb =
			(guint64)si.totalswap * si.mem_unit / 1024;
...
static gboolean parse_stat_line(const gchar *line, guint64 *idle,
				guint64 *total)
{
	gint64 fields[10];
	gsize n = 0;
...
void cpu_usage_sample(CpuSample *prev, gdouble *per_core, gsize n_cores,
		      gdouble *overall)
{
	gchar *stat;
	gchar **lines;
	...
	for (gsize i = 0; lines[i] != NULL; i++) {
		...
		if (per_core != NULL && prev != NULL && core < n_cores &&
		    prev[core].total > 0 && total > prev[core].total) {
