...
static void add_env_args(GPtrArray *args)
{
	const gchar *keys[] = { "DISPLAY",	   "WAYLAND_DISPLAY",
...
static void report_elevation_ready(void)
{
	const gchar *fd_str;
	gchar b = 'R';
	gssize nwritten;
...
static gboolean try_elevate(const gchar *pkexec, int *exit_status)
{
	GPtrArray *args;
	gchar *exe, *fd_arg;
	gint fds[2];
	...
	*exit_status = 0;
	...
	if (pid == 0) {
		...
		log_error("exec %s failed: %s", pkexec, g_strerror(errno));
		_exit(127);
	}
	close(fds[1]);
...
static void elevate_if_needed(void)
{
	gchar *pkexec;
	int exit_status = 0;
