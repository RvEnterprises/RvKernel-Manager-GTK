...
static void cpi_state_free(gpointer data)
{
	CPIState *state = data;
...
static gboolean cpi_tick(GtkWidget *widget, GdkFrameClock *clock,
			 gpointer user_data)
{
	CPIState *state;
	gdouble t;
	...
	t = 1.0 - pow(1.0 - t, 3.0);
	state->fraction =
		state->start_frac + (state->target - state->start_frac) * t;
	gtk_widget_queue_draw(widget);

	return G_SOURCE_CONTINUE;
}

static void cpi_build_layouts(CPIState *state, GtkWidget *widget, gint side,
			      const gchar *value, const gchar *caption)
{
	...
	state->font_stamp = base != NULL ? pango_font_description_hash(base) :
					   0;
	...
	pango_layout_get_pixel_size(state->caption_layout, &state->caption_w,
				    &state->caption_h);
...
static void cpi_update_value_text(CPIState *state, const gchar *value)
{
	g_free(state->layout_value);
	state->layout_value = g_strdup(value);
...
static void cpi_draw(GtkDrawingArea *area, cairo_t *cr, gint width, gint height,
		     gpointer user_data)
{
	...
	stamp = font_desc != NULL ? pango_font_description_hash(font_desc) : 0;
	...
	if (state->fraction > 0.001) {
		...
		cairo_arc(cr, center_x, center_y, radius, -M_PI / 2.0,
			  -M_PI / 2.0 + sweep);
		cairo_stroke(cr);
	}
	...
	{
		gdouble block_h =
			state->value_h + side * 0.02 + state->caption_h;
		gdouble text_y = center_y - block_h / 2.0;

		gdk_cairo_set_source_rgba(cr, &color);
		cairo_move_to(cr, center_x - state->value_w / 2.0, text_y);
...
GtkWidget *circular_progress_indicator_new(const gchar *caption)
{
	...
	gtk_drawing_area_set_content_width(GTK_DRAWING_AREA(widget), 148);
	gtk_drawing_area_set_content_height(GTK_DRAWING_AREA(widget), 148);
	gtk_drawing_area_set_draw_func(GTK_DRAWING_AREA(widget), cpi_draw, NULL,
				       NULL);

	g_object_set_data_full(G_OBJECT(widget), KEY_STATE, g_new0(CPIState, 1),
			       cpi_state_free);
...
void circular_progress_indicator_set_fraction(GtkWidget *self, gdouble fraction)
{
	CPIState *state;
	...
	if (state->tick_id == 0)
		state->tick_id = gtk_widget_add_tick_callback(self, cpi_tick,
							      NULL, NULL);
}

void circular_progress_indicator_set_text(GtkWidget *self, const gchar *fmt,
					  ...)
{
	CPIState *state;
	va_list args;
