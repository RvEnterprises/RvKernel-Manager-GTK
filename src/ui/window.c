...
static void ctx_free(gpointer data)
{
	WindowCtx *ctx = data;
...
static void refresh_current(WindowCtx *ctx)
{
	stack_refresh_visible(ctx->stack);
}

static gboolean on_screen(WindowCtx *ctx)
{
	GdkSurface *surface;
...
static void on_visible_child_changed(GtkStack *stack, GParamSpec *pspec,
				     gpointer user_data)
{
	(void)pspec;
	log_debug("visible page: %s", gtk_stack_get_visible_child_name(stack));
	refresh_current(user_data);
}

static void add_page(GtkStack *stack, GtkWidget *page, const gchar *name,
		     const gchar *title, const gchar *icon)
{
	...
	g_object_set(stack_page, "title", title, "icon-name", icon, NULL);
	log_debug("page '%s' (%s) registered", name, title);
}

static gboolean theme_name_is_dark(void)
{
 ...
 */
static gchar *invoking_state_dir(void)
{
	...
	if (geteuid() != 0) {
		...
		if (env != NULL && env[0] != '\0')
			return g_strdup(env);
		return g_build_filename(g_get_home_dir(), ".local", "state",
					NULL);
	}
	...
	uid = (uid_t)g_ascii_strtoull(runtime_dir + strlen("/run/user/"), NULL,
				      10);
	if (uid == 0)
		return NULL;
...
static gboolean json_bool_value(const gchar *json, const gchar *key,
				gboolean *value)
{
	const gchar *p;
 ...
 */
static gboolean dms_read_dark(gboolean *dark)
{
	...
	path = g_build_filename(state_dir, "DankMaterialShell", "session.json",
				NULL);
	g_free(state_dir);
...
static void scheme_watch_free(SchemeWatch *watch)
{
...
static void portal_scheme_done(GObject *source, GAsyncResult *result,
			       gpointer user_data)
{
	...
	if (watch->button != NULL &&
	    gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(watch->button)) !=
		    (scheme == 1))
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(watch->button),
					     scheme == 1);

	scheme_watch_free(watch);
}

static void session_bus_ready(GObject *source, GAsyncResult *result,
			      gpointer user_data)
{
	GDBusConnection *bus;
	SchemeWatch *watch = user_data;
	...
	g_dbus_connection_call(
		...
		g_variant_new("(ss)", "org.freedesktop.appearance",
			      "color-scheme"),
		G_VARIANT_TYPE("(v)"), G_DBUS_CALL_FLAGS_NO_AUTO_START, 300,
		NULL, portal_scheme_done, watch);
}

static void portal_scheme_async(GtkWidget *button)
{
	SchemeWatch *watch;
	...
	watch->button = button;
	g_object_add_weak_pointer(G_OBJECT(button), (gpointer *)&watch->button);

	g_bus_get(G_BUS_TYPE_SESSION, NULL, session_bus_ready, watch);
}

static void on_dark_toggled(GtkToggleButton *button, gpointer user_data)
{
	GtkSettings *settings = gtk_settings_get_default();
...
static GtkWidget *build_headerbar(WindowCtx *ctx, GtkWidget *window)
{
	...
	gtk_label_set_markup(GTK_LABEL(name_label),
			     g_markup_printf_escaped("<b><big>%s</big></b>",
						     APP_NAME));
	...
	{
		GtkWidget *icon =
			gtk_image_new_from_icon_name("dark-mode-symbolic");
...
static GtkWidget *build_banner(void)
{
...
GtkWidget *window_new(GtkApplication *app)
{
	...
	g_object_set_data_full(G_OBJECT(window), "window-ctx", ctx, ctx_free);
	...
	add_page(GTK_STACK(stack), page_dashboard_new(window), "dashboard",
		 "Dashboard", "dashboard-symbolic");
	add_page(GTK_STACK(stack), page_cpu_new(window), "cpu", "CPU",
		 "cpu-symbolic");
	add_page(GTK_STACK(stack), page_gpu_new(window), "gpu", "GPU",
		 "gpu-symbolic");
	add_page(GTK_STACK(stack), page_battery_new(window), "battery",
		 "Battery", "battery-symbolic");
	add_page(GTK_STACK(stack), page_memory_new(window), "memory", "Memory",
		 "memory-symbolic");
	add_page(GTK_STACK(stack), page_about_new(window), "about", "About",
		 "info-symbolic");
	...
	gtk_revealer_set_transition_type(
		GTK_REVEALER(revealer), GTK_REVEALER_TRANSITION_TYPE_CROSSFADE);
...
static gboolean hide_toast(gpointer user_data)
{
	WindowCtx *ctx = user_data;
...
void window_show_toast(GtkWidget *window, const gchar *message)
{
	...
	ctx->toast_timeout_id = g_timeout_add_seconds(4, hide_toast, ctx);
}
