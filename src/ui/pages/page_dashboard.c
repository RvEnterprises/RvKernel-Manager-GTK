...
#include "../components/circular_progress_indicator.h"
...
static gchar *mem_line(guint64 total_kb, guint64 avail_kb)
{
	guint64 used_kb = total_kb > avail_kb ? total_kb - avail_kb : 0;
	gchar *used_s = format_bytes(used_kb * 1024);
	gchar *total_s = format_bytes(total_kb * 1024);
...
static gchar *usage_line(guint64 used_bytes, guint64 total_bytes)
{
	gchar *used_s = format_bytes(used_bytes);
	gchar *total_s = format_bytes(total_bytes);
	gchar *out = g_strdup_printf("%s / %s", used_s, total_s);
...
static void set_severity(GtkWidget *widget, gdouble value)
{
	gtk_widget_remove_css_class(widget, "sev-warn");
	gtk_widget_remove_css_class(widget, "sev-crit");

	if (value >= 0.85)
		gtk_widget_add_css_class(widget, "sev-crit");
	else if (value >= 0.60)
		gtk_widget_add_css_class(widget, "sev-warn");
}

static void update_ring(GtkWidget *cpi, GtkWidget *detail, gdouble frac,
			guint64 used_bytes, guint64 total_bytes)
{
	gchar *text;

	circular_progress_indicator_set_fraction(cpi, frac);
	set_severity(cpi, frac);
...
static void refresh(GtkWidget *page)
{
	...
	ctx = g_object_get_data(G_OBJECT(page), "ctx");
	if (ctx == NULL)
		return;
	...
	{
		gchar *uptime = format_uptime(info->uptime_s);
		kv_set(GTK_WIDGET(ctx->uptime_row), "%s", uptime);
		g_free(uptime);
	}

	per_core = g_new0(gdouble, ctx->n_cores + 1);
	cpu_usage_sample(ctx->samples, per_core, ctx->n_cores, &overall);
	g_free(per_core);

	overall = CLAMP(overall < 0 ? 0 : overall, 0.0, 1.0);
	circular_progress_indicator_set_fraction(ctx->cpu_cpi, overall);
	set_severity(ctx->cpu_cpi, overall);

	if (info->mem_total_kb > 0) {
		gdouble frac = CLAMP(
			(gdouble)(info->mem_total_kb - info->mem_available_kb) /
				(gdouble)info->mem_total_kb,
			0.0, 1.0);

		circular_progress_indicator_set_fraction(ctx->ram_cpi, frac);
		circular_progress_indicator_set_text(ctx->ram_cpi, "%d%%",
						     (gint)(frac * 100));
		set_severity(ctx->ram_cpi, frac);
...

	{
		...
		gtk_widget_set_visible(GTK_WIDGET(ctx->zram_section), have);
		if (have) {
			update_ring(ctx->zram_cpi, ctx->zram_detail,
				    CLAMP((gdouble)used / (gdouble)size, 0.0,
					  1.0),
				    used, size);
		}
	}
	...
	if (info->disk_swap_total_kb > 0) {
		update_ring(ctx->swap_cpi, ctx->swap_detail,
			    CLAMP((gdouble)info->disk_swap_used_kb /
					  (gdouble)info->disk_swap_total_kb,
				  0.0, 1.0),
			    info->disk_swap_used_kb * 1024ULL,
			    info->disk_swap_total_kb * 1024ULL);
	} else {
		gtk_widget_set_visible(GTK_WIDGET(ctx->swap_section), FALSE);
	}

	for (gsize i = 0; i < ctx->battery_rows->len; i++) {
		DashBatteryRow *br = g_ptr_array_index(ctx->battery_rows, i);

		power_supply_refresh(br->ps);
		kv_set(br->info_row, "%d %% (%s)", br->ps->capacity,
		       br->ps->status != NULL ? br->ps->status : "?");
		gtk_level_bar_set_value(
			br->level_bar, CLAMP(br->ps->capacity, 0, 100) / 100.0);
	}

	system_info_free(info);
}

static GtkWidget *build_system_card(DashCtx *ctx)
{
	GtkWidget *card, *row;
	SystemInfo *info = system_info_get();

	card = card_new("System");

	row = kv_row("Hostname");
	card_add(card, row);
	kv_set(row, "%s", info->hostname);

	row = kv_row("Operating system");
	card_add(card, row);
	kv_set(row, "%s", info->distro);

	row = kv_row("Kernel");
	card_add(card, row);
	kv_set(row, "%s", info->kernel);

	row = kv_row("Architecture");
	card_add(card, row);
	kv_set(row, "%s", info->arch);

	row = kv_row("Processor");
	card_add(card, row);
	kv_set(row, "%s (%zu cores)", info->cpu_model, info->n_cores);
...
static GtkWidget *cpi_column(GtkWidget *cpi, GtkWidget *detail)
{
	GtkWidget *column = gtk_box_new(GTK_ORIENTATION_VERTICAL, 6);

	gtk_widget_set_halign(column, GTK_ALIGN_CENTER);
	gtk_widget_set_valign(cpi, GTK_ALIGN_START);
	gtk_box_append(GTK_BOX(column), cpi);
...
static GtkWidget *build_monitor_card(DashCtx *ctx)
{
	GtkWidget *card, *grid;

	card = card_new("Live monitor");
	...
	gtk_widget_add_css_class(grid, "section");
	gtk_widget_set_hexpand(grid, TRUE);
	gtk_widget_set_valign(grid, GTK_ALIGN_START);

	ctx->cpu_cpi = circular_progress_indicator_new("CPU");
	gtk_flow_box_append(GTK_FLOW_BOX(grid), cpi_column(ctx->cpu_cpi, NULL));

	ctx->ram_detail = gtk_label_new("-");
	ctx->ram_cpi = circular_progress_indicator_new("RAM");
	gtk_flow_box_append(GTK_FLOW_BOX(grid),
			    cpi_column(ctx->ram_cpi, ctx->ram_detail));

	ctx->zram_detail = gtk_label_new("-");
	ctx->zram_cpi = circular_progress_indicator_new("ZRAM");
	ctx->zram_section = cpi_column(ctx->zram_cpi, ctx->zram_detail);
	gtk_flow_box_append(GTK_FLOW_BOX(grid), ctx->zram_section);

	ctx->swap_detail = gtk_label_new("-");
	ctx->swap_cpi = circular_progress_indicator_new("Swap");
	ctx->swap_section = cpi_column(ctx->swap_cpi, ctx->swap_detail);
...
static GtkWidget *build_battery_card(DashCtx *ctx)
{
	GtkWidget *card = NULL;
	PowerSupply **supplies = power_supply_list(&ctx->n_supplies);

	ctx->supplies = supplies;
	ctx->battery_rows = g_ptr_array_new_with_free_func(g_free);

	for (gsize i = 0; i < ctx->n_supplies; i++) {
		PowerSupply *ps = supplies[i];
		DashBatteryRow *br;

		if (ps->kind != PS_BATTERY || ps->capacity < 0)
			continue;

		if (card == NULL)
			card = card_new("Battery");

		br = g_new0(DashBatteryRow, 1);
		br->ps = ps;

		br->info_row = kv_row(ps->name);
		card_add(card, br->info_row);
...
GtkWidget *page_dashboard_new(GtkWidget *window)
{
	...
	gtk_box_append(GTK_BOX(content), build_monitor_card(ctx));
