...
static void on_governor_selected(GtkDropDown *dropdown, GParamSpec *pspec,
				 gpointer user_data)
{
...
static void on_epp_selected(GtkDropDown *dropdown, GParamSpec *pspec,
			    gpointer user_data)
{
	PolicyUi *ui = user_data;
...
static GtkWidget *build_policy_card(CpuPolicy *policy, PolicyUi *ui)
{
	GtkWidget *card, *row;
	gchar *card_title;
	gchar *header;
	...
	ui->cur_freq_path =
		g_build_filename(policy->path, "scaling_cur_freq", NULL);
	...
	if (policy->governors[0] != NULL) {
		...
		card_add(card, row);
		kv_set(row, "%s",
		       policy->governor != NULL ? policy->governor : "-");
	}
	...
	if (policy->freqs_khz != NULL && policy->n_freqs >= 2) {
		...
		if (policy->min_freq_khz != NULL)
			option_row_select_id(min_row, policy->min_freq_khz);
		if (policy->max_freq_khz != NULL)
			option_row_select_id(max_row, policy->max_freq_khz);
