...
static void on_limit_changed(GtkRange *range, gpointer user_data)
{
...
static void on_limit_apply(GtkButton *button, gpointer user_data)
{
	...
	if (!power_supply_set_charge_limit(ui->ps, percent, &error)) {
		...
		log_error("battery page: limit -> %d%% failed: %s", percent,
			  window_error_text(&error));
...
static GtkWidget *build_charge_limit_row(BatteryUi *ui)
{
	...
	scale = gtk_scale_new_with_range(GTK_ORIENTATION_HORIZONTAL, 40, 100,
					 5);
	gtk_range_set_value(GTK_RANGE(scale), ui->ps->charge_limit > 0 ?
						      ui->ps->charge_limit :
						      100.0);
	...
	g_signal_connect(scale, "value-changed", G_CALLBACK(on_limit_changed),
			 ui->limit_value_label);
...
static GtkWidget *build_battery_card(PowerSupply *ps, BatteryUi *ui)
{
	GtkWidget *card;
...
static void refresh_battery_ui(BatteryUi *ui)
{
	...
	if (ps->has_health && ps->health >= 0)
		kv_set(ui->health_row, "%d%% (%.1f Wh / %.2f Wh design)",
		       ps->health, ps->charge_full_wh, ps->charge_design_wh);
	else
	...
	if (ui->energy_row != NULL)
		kv_set(ui->energy_row, "%.2f / %.2f Wh", ps->charge_full_wh,
		       ps->charge_design_wh);
}
...
GtkWidget *page_battery_new(GtkWidget *window)
{
	...
	for (gsize i = 0; i < ctx->n_supplies; i++) {
		...
		gtk_box_append(GTK_BOX(content), build_battery_card(ps, ui));
	...
	if (!any) {
		GtkWidget *label =
			gtk_label_new("No battery or AC adapter detected.");
