...
static GtkWidget *make_kv(GtkWidget *card, const gchar *key)
{
...
static void show_write_error(GtkWidget *window, GError **error)
{
	...
	log_error("memory page: apply failed: %s", window_error_text(error));
	window_show_toast(window, msg);
	g_free(msg);
}

static void on_vm_tunable_selected(GtkSpinButton *spin, gpointer user_data)
{
...
static void on_zram_algo_selected(GtkDropDown *dropdown, GParamSpec *pspec,
				  gpointer user_data)
{
	ZramUi *ui = user_data;
	...
	if (!zram_set_algo(ui->zram, id, &error)) {
		...
		gchar *msg =
			g_strdup_printf("zram algorithm set to %s "
					"(takes effect on next device reset)",
					id);
...
static void on_tcp_selected(GtkDropDown *dropdown, GParamSpec *pspec,
			    gpointer user_data)
{
...
static GtkWidget *build_vm_card(GtkWidget *window)
{
	GtkWidget *card = NULL;
...
static void tag_dropdown(GtkWidget *row)
{
	g_object_set_data(G_OBJECT(option_row_dropdown(row)), "row-ref", row);
}

static GtkWidget *build_zram_card(GtkWidget *window, Zram *zram, ZramUi *ui)
{
	GtkWidget *card;
	...
	if (zram->algos != NULL && zram->algos[0] != NULL) {
		...
		for (gsize i = 0; zram->algos[i] != NULL; i++)
			option_row_append(row, zram->algos[i], zram->algos[i]);
		if (zram->algo != NULL)
...
static GtkWidget *build_tcp_card(GtkWidget *window)
{
	gsize n_cc = 0;
	...
	if (n_cc > 0 && current != NULL) {
		GtkWidget *row = option_row_new("TCP congestion control");
...
static void refresh(GtkWidget *page)
{
	...
	for (gsize i = 0; i < ctx->zram_uis->len; i++) {
		...
		if (z->has_stats) {
			...
			kv_set(ui->usage_row, "%s (data %s → %s)", used, orig,
			       compr);
			g_free(used);
			g_free(orig);
			g_free(compr);
