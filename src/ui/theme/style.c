...
static gchar *build_css(const MaterialPalette *p)
{
	return g_strdup_printf(
		...
		"  color: %s;"
		"}",
		p->surface, p->on_surface, p->surface, p->on_surface,
		p->on_surface_variant, p->surface_container, p->outline_variant,
		p->on_surface_variant, p->secondary_container,
		p->on_secondary_container, p->primary, p->warn, p->error,
		p->on_surface, p->primary, p->surface_container, p->on_surface,
		p->secondary_container, p->on_secondary_container);
}

void style_set_dark(gboolean dark)
{
	...
	if (active_provider != NULL) {
		gtk_style_context_remove_provider_for_display(
			display, GTK_STYLE_PROVIDER(active_provider));
