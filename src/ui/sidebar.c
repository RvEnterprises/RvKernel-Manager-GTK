...
static void ctx_free(gpointer data)
{
	...
	if (self->stack != NULL) {
		g_signal_handler_disconnect(gtk_stack_get_pages(self->stack),
...
static void update_row(GtkWidget *row)
{
...
static void page_notify_cb(GObject *obj, GParamSpec *pspec, gpointer user_data)
{
...
static GtkWidget *create_row(GtkStackPage *page)
{
	...
	g_object_set_data_full(G_OBJECT(row), "page", g_object_ref(page),
			       g_object_unref);
	g_signal_connect_object(page, "notify", G_CALLBACK(page_notify_cb), row,
				0);
	update_row(row);
	return row;
}

static void sync_selection(Sidebar *self)
{
	...
	for (row = gtk_widget_get_first_child(GTK_WIDGET(self->list));
		...
		if (page != NULL && gtk_stack_page_get_child(page) == visible) {
...
static void visible_child_cb(GObject *obj, GParamSpec *pspec,
			     gpointer user_data)
{
...
static void row_selected_cb(GtkListBox *list, GtkListBoxRow *row,
			    gpointer user_data)
{
	Sidebar *self = user_data;
	GtkStackPage *page;
...
static void rebuild_rows(Sidebar *self)
{
...
static void items_changed_cb(GListModel *model, guint position, guint removed,
...
GtkWidget *sidebar_new(void)
{
	...
	g_signal_connect(list, "row-selected", G_CALLBACK(row_selected_cb),
			 self);

	return list;
}

void sidebar_set_stack(GtkWidget *widget, GtkStack *stack)
{
	...
	self->items_changed_id =
		g_signal_connect(gtk_stack_get_pages(stack), "items-changed",
